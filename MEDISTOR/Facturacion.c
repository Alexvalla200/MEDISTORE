...
void Adicionar_Compra(int *Num_med, char Name_med[][MAXIMO], int cant[], float precio[], float descuento[], int *total_productos, float *valor_total) {
    ...
    if (archivo == NULL) {
        printf("No se pudo abrir el archivo de medicamentos.\n");
        return;
    }
    ...
    while (fgets(linea, sizeof(linea), archivo)) {
        if (strstr(linea, "Nombre:") != NULL) {
            ...
            sscanf(linea, "Nombre: %s", nombre_archivo);
            if (strcmp(nombre_archivo, nombre) == 0) {
                encontrado = linea_actual;
                break;
            }
        }
        linea_actual++;
    ...
    if (encontrado == -1) {
        printf("Medicamento no encontrado en el archivo.\n");
        fclose(archivo);
        return;
    }
    ...
    if (fgets(linea, sizeof(linea), archivo) != NULL) {
        sscanf(linea, "Cantidad: %d", &cant[encontrado]);
    } else {
        printf("Error al leer la cantidad actual del medicamento.\n");
        fclose(archivo);
        return;
    }

    if (fgets(linea, sizeof(linea), archivo) != NULL) {
        sscanf(linea, "Precio unitario: %f", &precio[encontrado]);
    } else {
        printf("Error al leer el precio unitario del medicamento.\n");
        fclose(archivo);
        return;
    }

    if (fgets(linea, sizeof(linea), archivo) != NULL) {
        sscanf(linea, "Porcentaje Descuento: %f", &descuento[encontrado]);
    } else {
        printf("Error al leer el porcentaje de descuento del medicamento.\n");
        fclose(archivo);
        return;
    }
